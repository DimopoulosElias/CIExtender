...
def set_INFERENCE_CHARS(dbms):
    global INFERENCE_EQUALS_CHAR
    global INFERENCE_GREATER_CHAR
    global INFERENCE_NOT_EQUALS_CHAR

    if dbms == DBMS.SHELL:
        INFERENCE_EQUALS_CHAR = ' -eq '
        INFERENCE_GREATER_CHAR = ' -gt '
        INFERENCE_NOT_EQUALS_CHAR = ' -nq '
    elif dbms == DBMS.PHP or dbms == DBMS.PYTHON:
        INFERENCE_EQUALS_CHAR = ' == '
    else:
        INFERENCE_EQUALS_CHAR = '='
...
def checkSqlInjection(place, parameter, value):
    ...
    while tests:
        ...
        try:
            ...
            # Addition to control the INFERENCE_*_CHARs
            for key in test:
                 if key == 'details':
                    dbms = test['details']['dbms']
                    set_INFERENCE_CHARS(dbms)
             # Addition END
    ...
    # Addition to control the INFERENCE_*_CHARs
    set_INFERENCE_CHARS(Backend.getDbms())
    # Addition END
...
def checkFilteredChars(injection):
    ...
    if len(injection.data) == 1 and PAYLOAD.TECHNIQUE.BOOLEAN in injection.data:
        if not checkBooleanExpression("(%d)%s%d" % (randInt, INFERENCE_EQUALS_CHAR, randInt)):
    ...
    if not any(_ in injection.data for _ in (PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.UNION, PAYLOAD.TECHNIQUE.QUERY)):
        if not checkBooleanExpression("%d%s%d" % (randInt + 1, INFERENCE_GREATER_CHAR, randInt)):
