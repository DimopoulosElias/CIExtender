...
from lib.core.settings import SHELL_ALIASES
from lib.request import inject
from plugins.generic.fingerprint import Fingerprint as GenericFingerprint

class Fingerprint(GenericFingerprint):
    def __init__(self):
        GenericFingerprint.__init__(self, DBMS.SHELL)

    def _commentCheck(self):
        infoMsg = "executing %s comment injection fingerprint" % DBMS.SHELL
        logger.info(infoMsg)

        result = inject.checkBooleanExpression("[RANDNUM]=[RANDNUM]/* NoValue */")

        if not result:
            warnMsg = "unable to perform %s comment injection" % DBMS.SHELL
            logger.warn(warnMsg)
    ...
    def checkDbms(self):
        ...
        * http://dev.mysql.com/doc/refman/6.0/en/news-6-0-x.html (manual has been withdrawn)
        ...
        result = inject.checkBooleanExpression('$(expr substr "$(id)" 1 1) = "u"')

        if result:
            infoMsg = "confirming %s command injection" % DBMS.SHELL
            logger.info(infoMsg)

            result = inject.checkBooleanExpression('$(expr substr "$(pwd)" 1 1) = "/"')
    ...
    def checkDbmsOs(self, detailed=False):
        ...
        result = inject.checkBooleanExpression("\"L\"===strtoupper(substr(SHELL_OS,0,1))")
