...
class Filesystem(GenericFilesystem):
    ...
    def stackedReadFile(self, remoteFile):
        ...
        length = inject.getValue("strlen(bin2hex(file_get_contents(\"%s\")))" % (remoteFile), resumeValue=False, expected=EXPECTED.INT, charsetType=CHARSET_TYPE.DIGITS)
        ...
        if not isNumPosStrValue(length):
            ...
            if length > chunkSize:
                ...
                for i in xrange(0, length, chunkSize):
                    chunk = inject.getValue("substr(bin2hex(file_get_contents(\"%s\")),%s,%s)" % (remoteFile, i, chunkSize), unpack=False, resumeValue=False, charsetType=CHARSET_TYPE.HEXADECIMAL)
                    result.append(chunk)
            else:
