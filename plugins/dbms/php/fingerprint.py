...
from lib.core.settings import PHP_ALIASES
from lib.request import inject
from plugins.generic.fingerprint import Fingerprint as GenericFingerprint

class Fingerprint(GenericFingerprint):
    def __init__(self):
        GenericFingerprint.__init__(self, DBMS.PHP)

    def _commentCheck(self):
        infoMsg = "executing %s comment injection fingerprint" % DBMS.PHP
        logger.info(infoMsg)

        result = inject.checkBooleanExpression("[RANDNUM]=[RANDNUM]/* NoValue */")

        if not result:
            warnMsg = "unable to perform %s comment injection" % DBMS.PHP
            logger.warn(warnMsg)
    ...
    def checkDbms(self):
        ...
        * http://dev.mysql.com/doc/refman/6.0/en/news-6-0-x.html (manual has been withdrawn)
        ...
        if not conf.extensiveFp and Backend.isDbmsWithin(PHP_ALIASES):
            setDbms("%s %s" % (DBMS.PHP, Backend.getVersion()))
        ...
        infoMsg = "testing %s command injection" % DBMS.PHP
        logger.info(infoMsg)

        result = inject.checkBooleanExpression("substr((\"test\"),0,1) === \"t\"")

        if result:
            infoMsg = "confirming %s command injection" % DBMS.PHP
            logger.info(infoMsg)

            result = inject.checkBooleanExpression("version_compare(phpversion(), 0) === 1")
            ...
            if not result:
                warnMsg = "the back-end DBMS is not %s" % DBMS.PHP
                logger.warn(warnMsg)
            ...
            if inject.checkBooleanExpression("version_compare(phpversion(), \"7.0\") >= 0"):
                kb.data.has_information_schema = True
                Backend.setVersion(">= 7.0")
                setDbms("%s 7" % DBMS.PHP)
                self.getBanner()
            ...
            # Determine if it is MySQL >= 5.0.0
                ...
                setDbms("%s 5" % DBMS.PHP)
                self.getBanner()

                if not conf.extensiveFp:
                    return True

                infoMsg = "actively fingerprinting %s" % DBMS.PHP
                logger.info(infoMsg)
    ...
    def checkDbmsOs(self, detailed=False):
        ...
        result = inject.checkBooleanExpression("\"L\"===strtoupper(substr(PHP_OS,0,1))")
