...
from lib.core.settings import PYTHON_ALIASES
from lib.request import inject
from plugins.generic.fingerprint import Fingerprint as GenericFingerprint

class Fingerprint(GenericFingerprint):
    def __init__(self):
        GenericFingerprint.__init__(self, DBMS.PYTHON)

    def _commentCheck(self):
        infoMsg = "executing %s comment injection fingerprint" % DBMS.PYTHON
        logger.info(infoMsg)

        result = inject.checkBooleanExpression("[RANDNUM]=[RANDNUM]/* NoValue */")

        if not result:
            warnMsg = "unable to perform %s comment injection" % DBMS.PYTHON
            logger.warn(warnMsg)
    ...
    def checkDbms(self):
        ...
        * http://dev.mysql.com/doc/refman/6.0/en/news-6-0-x.html (manual has been withdrawn)
        ...
        if not conf.extensiveFp and Backend.isDbmsWithin(PYTHON_ALIASES):
            setDbms("%s %s" % (DBMS.PYTHON, Backend.getVersion()))
        ...
        infoMsg = "testing %s command injection" % DBMS.PYTHON
        logger.info(infoMsg)

        result = inject.checkBooleanExpression("\"test\"[0] == \"t\"")

        if result:
            infoMsg = "confirming %s command injection" % DBMS.PYTHON
            logger.info(infoMsg)
            ...
            if not result:
                warnMsg = "the back-end DBMS is not %s" % DBMS.PYTHON
                logger.warn(warnMsg)
            ...
            if inject.checkBooleanExpression("version_compare(pythonversion(), \"7.0\") >= 0"):
                kb.data.has_information_schema = True
                Backend.setVersion(">= 7.0")
                setDbms("%s 7" % DBMS.PYTHON)
                self.getBanner()
            ...
            # Determine if it is MySQL >= 5.0.0
                ...
                setDbms("%s 5" % DBMS.PYTHON)
                self.getBanner()

                if not conf.extensiveFp:
                    return True

                infoMsg = "actively fingerprinting %s" % DBMS.PYTHON
                logger.info(infoMsg)
