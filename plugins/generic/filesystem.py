...
class Filesystem(object):
    ...
    def _checkFileLength(self, localFile, remoteFile, fileRead=False):
        if Backend.isDbms(DBMS.MYSQL):
            ...
            lengthQuery = "strlen(file_get_contents(\"%s\"))" % remoteFile

        elif Backend.isDbms(DBMS.PYTHON):
            lengthQuery = "str(len(open(\"%s\").read()))" % remoteFile
        elif Backend.isDbms(DBMS.SHELL):
            lengthQuery = "expr length \"$(cat %s)\"" % remoteFile
    ...
    def readFile(self, remoteFile):
        ...
        for remoteFile in remoteFile.split(','):
            ...
            if conf.direct or isStackingAvailable():
                ...
                debugMsg = "going to read the file with "
                debugMsg += "PHP injection technique"
                logger.debug(debugMsg)

                fileContent = self.stackedReadFile(remoteFile)
            elif Backend.isDbms(DBMS.PYTHON):
                debugMsg = "going to read the file with "
                debugMsg += "PYTHON injection technique"
                logger.debug(debugMsg)

                fileContent = self.stackedReadFile(remoteFile)
            elif Backend.isDbms(DBMS.SHELL):
                debugMsg = "going to read the file with "
                debugMsg += "SHELL injection technique"
                logger.debug(debugMsg)
    ...
    def writeFile(self, localFile, remoteFile, fileType=None, forceCheck=False):
        ...
        if conf.direct or isStackingAvailable():
            ...
            debugMsg = "going to upload the file '%s' " % fileType
            logger.debug(debugMsg)
